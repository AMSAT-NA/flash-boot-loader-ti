		...
		while (file_done == 0) {
			...
			case GetPacketData:
				*packet_data = (char) key;  //data[0] = SOH, STX
				 ...
				 */
				for (i = 1; i < (packet_size + PACKET_HEADER + PACKET_CRC);
						i++) {
					key = UART_getChar(sci, WAIT_TIMEOUT);
					if (key < 0) {
						state = PacketErrors;
				...
				if (state == PacketErrors) {
					break;
				}
				// Locations 1 and 2 (zero based) should always add up to 0xff
				if ((packet_data[PACKET_SEQNUM_LOC]
						+ packet_data[PACKET_SEQNUM_LOC + 1]) != 255) {
					state = PacketErrors;
					break;
				}
				if (_cal_crc16_(packet_data + PACKET_HEADER,
						packet_size + PACKET_CRC) != 0) {
					state = PacketErrors;
					break;
				}
			...
			case Packet0:
				/* Filename packet: Name is not used for anything,
				 * as we don't have a file system, but size is useful
				 */
				if (packet_data[PACKET_HEADER] != 0) /* The filename is not NULL*/
				{
					/* Filename packet has valid data */
					for (i = 0, pFile = packet_data + PACKET_HEADER;
							(*pFile != 0) && (i < FILENAME_LEN);) {
						fileName[i++] = *pFile++;
					}
					fileName[i++] = '\0';
					...
					if (!BLInternalFlashStartAddrCheck(FlashDestination,
							imageSize)) {
						state = AbortHeader;
					}
					/* Initialize the Flash Wrapper registers */
					oReturnCheck = 0;
					/* Erase the FLASH pages, return error on access violation */
					oReturnCheck = Fapi_BlockErase(ucBank, FlashDestination,
							imageSize);
					if (oReturnCheck) {
						state = AbortHeader;
					}
			...
			case DataPacket:
				...
				oReturnCheck = Fapi_BlockProgram(ucBank, FlashDestination,
...
static void flush_input(sciBASE_t *sci) {
	while (UART_getChar(sci, WAIT_TIMEOUT) != -1)
		;
}
 ...
 * Return value:  None
 */
static void SendPacket(sciBASE_t *sci, unsigned char *data,
...
char Ymodem_Transmit(sciBASE_t *sci, unsigned char *pImage, char *fileName,
	...
	for (;;) {
		...
		if (fileName) { /*Packet0 like this: SOH 00 FF filename NULL filesize 00 00 00 ...*/
