...
uint32_t BLInternalFlashFirstSectorSizeGet(void) {
	...
	firstSectorSize = (uint32_t) (flash_sector[0].start)
			+ flash_sector[0].length;
...
uint32_t BLInternalFlashSizeGet(void) {
	uint32_t flashSize;
	flashSize = (uint32_t) flash_sector[NUMBEROFSECTORS - 1].start
			+ flash_sector[NUMBEROFSECTORS - 1].length;
	return (flashSize);
}
...
uint32_t BLInternalFlashStartAddrCheck(uint32_t ulAddr, uint32_t ulImgSize) {
	...
	//
	ulWholeFlashSize = (uint32_t) flash_sector[NUMBEROFSECTORS - 1].start
			+ flash_sector[NUMBEROFSECTORS - 1].length; /* 3MB */

	/* The start address must be at the begining of the sector */
	for (i = 0; i < NUMBEROFSECTORS; i++) {
		if ((ulAddr >= (uint32_t) (flash_sector[i].start))
				&& (ulAddr
						< ((uint32_t) flash_sector[i].start
								+ flash_sector[i].length))) {
			count++;
		}
	}
	if (count == 0) {
...
uint32_t Fapi_BlockErase(uint32_t Bank, uint32_t ulAddr, uint32_t Size) {
	...
	for (i = ucStartSector; i < NUMBEROFSECTORS; i++) {
		if (EndAddr
				<= (((uint32_t) flash_sector[i].start) + flash_sector[i].length)) {
...
uint32_t Fapi_BlockProgram(uint32_t Bank, uint32_t Flash_Address,
		uint32_t Data_Address, uint32_t SizeInBytes) {
	...
	if ((Fapi_initializeFlashBanks((uint32_t) SYS_CLK_FREQ))
			== Fapi_Status_Success) {
	...
	while (SizeInBytes > 0) {
		Fapi_issueProgrammingCommand((uint32_t*) dst, (uint8_t*) src,
				(uint32_t) bytes, 0, 0, Fapi_AutoEccGeneration);
...
uint32_t Fapi_UpdateStatusProgram(uint32_t Bank, uint32_t Flash_Start_Address,
		uint32_t Data_Start_Address, uint32_t Size_In_Bytes) {
	...
	Fapi_initializeAPI((Fapi_FmcRegistersType *)F021_CPU0_REGISTER_ADDRESS,
	...
	while ( Fapi_checkFsmForReady() == Fapi_Status_FsmBusy)
		;
	status = Flash_Program_Check(Flash_Start_Address, Data_Start_Address,
			Size_In_Bytes);
	return (status);
}

uint32_t Flash_Program_Check(uint32_t Program_Start_Address,
		uint32_t Source_Start_Address, uint32_t No_Of_Bytes) {
...
uint32_t Fapi_BlockRead(uint32_t Bank, uint32_t Flash_Start_Address,
		uint32_t Data_Start_Address, uint32_t Size_In_Bytes) {
	register uint32_t src = Data_Start_Address;
	register uint32_t dst = Flash_Start_Address;
